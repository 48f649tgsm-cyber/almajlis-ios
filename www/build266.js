/* BUILD 266 — category scrolling + player-career stock recovery */
(()=>{'use strict';
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelector('meta[name="almajlis-build"]')?.setAttribute('content','BUILD-266-CAREER-SCROLL-REPAIR-20260922');
  document.querySelector('meta[name="build-number"]')?.setAttribute('content','266');
  const badge=document.getElementById('buildBadge025');if(badge)badge.textContent='BUILD 266';
  document.querySelector('#draw .note')?.replaceChildren(document.createTextNode('الإصدار: BUILD 266'));
  try{sessionStorage.setItem('almajlis_build_seen','266');sessionStorage.setItem('almajlis_active_build','266')}catch(_){ }
 },{once:true});
})();
