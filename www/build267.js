...
(()=>{'use strict';
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelector('meta[name="almajlis-build"]')?.setAttribute('content','BUILD-267-CAREER-AUDIT-COVERS-20260922');
  document.querySelector('meta[name="build-number"]')?.setAttribute('content','267');
  const badge=document.getElementById('buildBadge025');if(badge)badge.textContent='BUILD 267';
  document.querySelector('#draw .note')?.replaceChildren(document.createTextNode('الإصدار: BUILD 267'));
  try{sessionStorage.setItem('almajlis_build_seen','267');sessionStorage.setItem('almajlis_active_build','267')}catch(_){ }
 },{once:true});
})();
